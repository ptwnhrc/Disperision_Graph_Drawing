...
clear;
close all; 
file=dir('F:\matlab_project\2021.09.02_ARR_PVSK_PhC\Data-txt\*.txt');
n_max = length(file);
y = []; y(1) = 0; y(2) = 173;
AA= 91; BB= -0.0618;  % y = a + b * x 边缘作直线拟合得到倾斜角度关系

Grating = 600 ; Center = 530;
xx_res =5000;  yy_res =1000;
NA = 0.8; n_air = 1;

 fr_bg = importdata('09991-BG-G600-c530-2s-10t_1.txt');  % 导入背景文件数据
% fr_Lamp = importdata('9001-Lamp-50X-g150-C620-500ms-10t_1.txt');   % 导入灯谱文件数据 9011-Lamp-50X-g150-C620-500ms-10t-P274_1
  fr_Lamp = importdata('09001-Ag-G600-c530-2s-10t_1.txt');   % 导入灯谱文件数据 9011-Lamp-50X-g150-C620-500ms-10t-P274_1
 
for n =1:n_max
  %strposition = strfind(file(n).name, '_g150_c570'); %仅导入反射光谱的数据
  strposition = strfind(file(n).name, '04003-S4-Ref-G600-c530-2s-10t-P146_1');
  ...
  %%提取对应角度上的吸收谱信息
         Abs = [0, 0.5, 1, 2, 3, 4, 6, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52];
             [~,Length_Abs]=size(Abs);
             fr_y_0 = yy_region(:,1);
             Spectra=[];
        if(1)
